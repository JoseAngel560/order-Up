...
router.get('/', async (req, res) => {
    try {
        const { restaurante_id, periodo, fechaInicio: customInicio, fechaFin: customFin, nombre_empleado, numero_mesa } = req.query;
        if (!restaurante_id) {
        ...
        switch (periodo) {
            ...
            case 'Personalizado':
                if (customInicio && customFin) {
                    fechaStart = new Date(customInicio + 'T00:00:00');
                    fechaEnd = new Date(customFin + 'T23:59:59.999');
                }
                else {
                    fechaStart.setHours(0, 0, 0, 0);
                }
        ...
        // --- 3. JOINs (Sin cambios) ---
        pipeline.push({ $lookup: { from: 'pedidos', localField: 'pedido_id', foreignField: '_id', as: 'pedido_info' } }, { $unwind: { path: '$pedido_info', preserveNullAndEmptyArrays: true } }, // preserveNull si una factura no tiene pedido (raro)
        { $lookup: { from: 'empleados', localField: 'mesero_id', foreignField: '_id', as: 'mesero_info' } }, { $unwind: { path: '$mesero_info', preserveNullAndEmptyArrays: true } }, { $lookup: { from: 'usuarios', localField: 'cajero_id', foreignField: '_id', as: 'cajero_info' } }, { $unwind: { path: '$cajero_info', preserveNullAndEmptyArrays: true } });
        // --- 4. APLICAR NUEVOS FILTROS (Sin cambios) ---
        if (numero_mesa) {
            pipeline.push({
                $match: { 'pedido_info.numero_mesa': parseInt(numero_mesa) }
            });
        }
        if (nombre_empleado) {
            const regex = new RegExp(nombre_empleado, 'i');
            pipeline.push({
                $match: {
                    $or: [
        ...
        const facturas = await Factura_1.default.aggregate(pipeline);

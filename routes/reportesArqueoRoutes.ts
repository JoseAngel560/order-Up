...
router.get('/', async (req, res) => {
    try {
        ...
        if (nombre_cajero) {
            filtros.cajero_nombre = { $regex: new RegExp(nombre_cajero as string, 'i') };
        }

        // --- 4. OBTENER ARQUEOS ---
        const arqueos = await Arqueo.find(filtros)
        ...
        for (const a of arqueos) {
            ...
            const monedaHist = a.moneda_base_historica as CurrencyType;
